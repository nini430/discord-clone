...
const ServerHeader = ({ role, server }: ServerHeaderProps) => {
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent
        style={{ width: '100%' }}
        className="text-background font-semibold w-full"
      >
        {isModerator && (
          <DropdownMenuItem
            onClick={() => onOpen('invite', { server })}
            className="text-indigo-600 w-full cursor-pointer dark:text-indigo-400 px-3 py-2"
          >
            Invite People
        ...
        {isAdmin && (
          <DropdownMenuItem
            ...
            className=" w-full text-white  cursor-pointer px-3 py-2"
          >
            Server Settings
        ...
        {isAdmin && (
          <DropdownMenuItem onClick={()=>onOpen('delete-server',{server})} className=" w-full  text-rose-500  cursor-pointer px-3 py-2">
            Delete Server
        ...
        {!isAdmin && (
          <DropdownMenuItem
            ...
            className=" w-full  text-rose-500  cursor-pointer px-3 py-2"
          >
            Leave Server
